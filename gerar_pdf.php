...
// Consulta SQL para obter os dados
$sql_code = "SELECT escolaridademae, COUNT(*) AS quantidade, CONCAT(FORMAT(COUNT(*) * 100 / SUM(COUNT(*)) OVER(), 2), '%') AS porcentagem FROM form3 GROUP BY escolaridademae; ";
$result = mysqli_query($mysqli, $sql_code);
...
// Preencher a tabela com os dados
while ($row = $result->fetch_assoc()) {
    $pdf->Cell(50, 10, $row['escolaridademae'], 1, 0, 'C');
    $pdf->Cell(70, 10, $row['quantidade'], 1, 0, 'C');
    $pdf->Cell(50, 10, $row['porcentagem'], 1, 1, 'C');
}
